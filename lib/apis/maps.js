...
// Build a canonical, shareable Google Maps directions URL between two points.
// Uses the documented Maps URL API (api=1) with lat,lng so routing is exact and
// the link opens directly in the visitor's Google Maps. Returns null if coords
// are missing. travelmode: walking | transit | driving | bicycling.
const TRAVELMODE = { walking: 'walking', transit: 'transit', driving: 'driving', bicycling: 'bicycling' };
export function buildDirectionsUrl({ originLat, originLng, destLat, destLng, mode = 'transit' } = {}) {
  if ([originLat, originLng, destLat, destLng].some((n) => typeof n !== 'number')) return null;
  const tm = TRAVELMODE[mode] ?? 'transit';
