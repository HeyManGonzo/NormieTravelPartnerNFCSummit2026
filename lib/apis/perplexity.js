 ...
 * @param {string|null} [options.language=null] — ISO code (e.g. 'pt') to steer Sonar toward
 *        sources in that language and the freshest local reporting. Use for time-sensitive
 *        local news where English coverage lags the local press.
...
const LANGUAGE_STEER = {
  pt: 'Prioritise the most recent reporting from authoritative Portuguese-language sources over '
    + 'slower international or English-language coverage, and report the situation as it stands today.',
};
...
export async function askPerplexity(query, {
  ...
  const langSteer = language ? (LANGUAGE_STEER[language] ?? null) : null;
  const ck = `${q}|${recency}|${contextSize}|${(domainFilter ?? []).join(',')}|${systemHint ?? ''}|${language ?? ''}`;
  const hit = cache.get(ck);
  if (hit && Date.now() - hit.at < CACHE_TTL_MS) return hit.value;

  const systemContent = [SYSTEM_BASE, systemHint, langSteer].filter(Boolean).join('\n\n');
  const body = {
    model: MODEL,
    messages: [
