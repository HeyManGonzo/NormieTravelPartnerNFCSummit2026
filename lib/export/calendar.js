...
// When the afternoon slot holds a restaurant (lunch on a relaxed-pace day),
// shift to a proper lunch window instead of mid-afternoon.
const LUNCH_WINDOW = { startH: 13, startM: 0, hours: 1.5 };
...
function blockTiming(date, block, nfcDay, programme) {
  ...
  const isLunch =
    block.timeSlot === 'afternoon' && block.activity?.type === 'restaurant';
  const win = isLunch
    ? LUNCH_WINDOW
    : SLOT_WINDOWS[block.timeSlot] ?? SLOT_WINDOWS.morning;
  return {
    start: [y, mo, d, win.startH, win.startM],
    duration: {
      hours: Math.floor(win.hours),
      minutes: Math.round((win.hours % 1) * 60),
    },
  };
}
...
function eventFromBlock(date, block, nfcDay, programme, msgs) {
  ...
  const evt = {
    ...
    productId: 'gemel-normie-6832/nfc-summit-2026',
