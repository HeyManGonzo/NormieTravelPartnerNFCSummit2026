...
function toActivity(c, opts = {}) {
  if (!c) return null;
  // Accept both flat lat/lng (retrieval result shape) and nested
  // coordinates.{lat,lng} (raw catalog JSON shape) so the function is
  // robust to either candidate source.
