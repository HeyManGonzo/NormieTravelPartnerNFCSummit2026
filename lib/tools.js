...
import { travelTimeBetween, buildDirectionsUrl } from '@/lib/apis/maps.js';
...
import { askPerplexity } from '@/lib/apis/perplexity.js';
...
export const TOOL_DEFINITIONS = [
  ...
  },
  {
    name: 'getTravelTime',
    description:
      'Estimate transit/walking time between two Lisbon venues AND get a shareable Google Maps directions link between them. Use it (a) when comparing options or checking whether two stops fit in one block, and (b) whenever the visitor asks for a route, directions, or a "map link" between two places. Resolves either named venues or any place via Google. Returns durationText/distanceText (when available) plus directionsUrl — present that URL to the visitor as a clickable markdown link, never as raw text.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'searchWeb',
    description:
      'Web search via Perplexity Sonar — returns a synthesised, source-cited answer (not raw links). One tool for three jobs: (1) editorial context the curated catalog, Tripadvisor, and Google Places cannot give you (Time Out / Eater / Condé Nast reviews, travel-blog roundups, "what does X publication recommend"); (2) time-sensitive news (transport strikes, weather warnings, venue closures, festival announcements) — set recency:"day" or "week"; (3) community sentiment and local wisdom ("what do locals actually think of X", "is Y a tourist trap") — set communityFocus:true to restrict to Reddit. Prefer searchKnowledge, searchPlaces, and searchTripadvisor first; reach for searchWeb when those come up short or the visitor names an editorial source. Lisbon-scoped by default. Returns { answer, citations }. Quote any address/hours verbatim from the answer — never infer specifics. Cite sources inline as [source](url) using the returned citations.',
    input_schema: {
      type: 'object',
      properties: {
        query: {
          type: 'string',
          description:
            'Free-text question, e.g. "Time Out best fado in Alfama", "transport strikes in Lisbon this week", "what do locals think of Time Out Market".',
        },
        recency: {
          type: 'string',
          enum: ['hour', 'day', 'week', 'month', 'year'],
          description: 'Optional. How recent results must be. Omitted by default (no recency filter). Set "day"/"week" for news, strikes, closures, or anything time-sensitive — this also steers the search toward fresh Portuguese-language sources and official transit operators.',
        },
        communityFocus: {
          type: 'boolean',
          description: 'Optional. Set true to restrict the search to Reddit for community sentiment / local opinions. Defaults to false.',
        },
        lisbonScope: {
          type: 'boolean',
          description: 'Optional. Defaults to true (scopes the query to Lisbon). Set to false for Portugal-wide questions, e.g. national rail strikes.',
        },
      },
      required: ['query'],
    },
...
// Common ways visitors and Gemel refer to the Summit venue. It's curated as
// the event entry named "NFC Summit 2026" (Unicorn Factory Lisboa, Beato), so
// these phrasings miss a name match and would otherwise fall through to a
// Google Places guess — which resolves to the wrong Unicorn Factory site in
// Alcântara. Map them to the authoritative event entry instead.
const SUMMIT_VENUE_ID = 'nfc-summit-2026';
const SUMMIT_VENUE_ALIASES = [
  'unicorn factory', 'nfc summit', 'summit venue', 'conference venue',
  'the venue', 'the summit',
];
...
async function findVenueByName(name) {
  ...
  // Match on the building/venue name (e.g. "Unicorn Factory Lisboa").
  if (!hit) {
    hit = catalog.find((e) => e.venue && needle.includes(e.venue.toLowerCase()));
  }
  // Resolve well-known Summit-venue aliases to the authoritative event entry.
  if (!hit && SUMMIT_VENUE_ALIASES.some((a) => needle.includes(a))) {
    hit = catalog.find((e) => e.id === SUMMIT_VENUE_ID);
  }
  if (hit) return hit;
...
async function execGetTravelTime(input) {
  ...
  // The directions link is the primary deliverable and doesn't depend on the
  // distance-matrix call, so build it from the resolved coordinates first.
  ...
  // Travel time is best-effort: include it when the Maps API answers, but a
  // failure there must not block the link.
  ...
  return {
    ...
    distanceText: hint?.distanceText ?? null,
  };
}
...
// For time-sensitive local news (strikes, closures, weather warnings, airport
// issues), the freshest and most accurate coverage is the Portuguese press and
// the transit operators themselves — international/English outlets lag. Steer
// Sonar toward them with a system hint + a bilingual query nudge. Kept a soft
// preference, not a hard search_domain_filter: a narrow domain filter often
// returns zero grounded results (see the communityFocus note below).
const LOCAL_NEWS_HINT =
  'This is a time-sensitive local-news question. Prioritise authoritative '
  + 'Portuguese-language sources (CNN Portugal, SIC Notícias, Público, Observador, '
  + 'Expresso, RTP, Lusa) and official operator updates (Metro de Lisboa, Carris, '
  + 'CP, ANA/Aeroportos) over international or English-language coverage, and lead '
  + 'with the most recent reporting.';

// recency values that signal a time-sensitive question worth the local-news steer.
const TIME_SENSITIVE = new Set(['hour', 'day', 'week']);

async function execSearchWeb(input) {
  if (!input?.query) return { error: 'query is required' };
  const timeSensitive = TIME_SENSITIVE.has(input.recency);
  // For community sentiment, nudge the query toward local/Reddit discussion
  // rather than a hard reddit.com domain filter — a single-domain filter often
  // returns zero results (ungrounded answer), whereas the nudge reliably
  // surfaces community sources while still grounding.
  let query = input.query;
  if (input.communityFocus) {
    query = `${input.query} — what do locals and Reddit users actually say?`;
  } else if (timeSensitive) {
    // Bilingual nudge so Sonar's retrieval surfaces same-day Portuguese coverage.
    query = `${input.query} (include today's Portuguese-language news coverage / notícias de hoje)`;
  }
  const result = await askPerplexity(query, {
    recency: input.recency ?? null, // omit unless the model set it (news/time-sensitive)
    systemHint: timeSensitive ? LOCAL_NEWS_HINT : null,
    language: timeSensitive ? 'pt' : null,
    lisbonScope: input.lisbonScope !== false,
  });
  if (!result) {
    return { available: false, reason: 'web search unavailable or no results' };
  }
  return {
    available: true,
