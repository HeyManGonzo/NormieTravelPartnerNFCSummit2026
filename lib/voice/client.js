...
function cleanTextForSpeech(text) {
  const cleaned = text
    ...
    .trim();
  // Hard cap: itinerary narrations can be very long. Truncate at a sentence
  // boundary to avoid ElevenLabs timeouts or garbled audio.
  if (cleaned.length <= TTS_MAX_CHARS) return cleaned;
  const cutoff = cleaned.lastIndexOf('.', TTS_MAX_CHARS);
  return cutoff > 0 ? cleaned.slice(0, cutoff + 1) : cleaned.slice(0, TTS_MAX_CHARS);
}
